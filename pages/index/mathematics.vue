	...
	<view class="content">
		<view class="box" v-if="isshow">
			<view v-if="text!='简答' && text!='名词解释' && text!='填空'">
				...
				<view class="title" v-if="list[index-1].title.indexOf('.jpg')==-1">
					{{index}}.{{list[index-1].title}}{{list[index-1].title==""?'听力题':""}}
				</view>
				<view v-else>
					{{index}}.
					<img style="width: 100%;height: 200rpx;" :src="list[index-1].title" alt="">
				</view>
			</view>
			...
			<view class="btn" v-if="text=='多选' && active == -1" @click="submit()">提交</view>
...
<script>
	...
	import mathematics from '../json/data.json'
	export default {
		data() {
			return {
				list: [],
		...
		onLoad(options) {
			if (options.text) {
				...
				if (options.text == "模拟考试") {
					...
					if (options.type == "数学") {
						if (mathematics[options.text] == undefined) {
							this.isshow = false
							return
						}
						this.list = this.getnewarr(mathematics[options.text])
					}
